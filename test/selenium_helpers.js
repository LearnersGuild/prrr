...
const Browser = function(){
    ...
    return this.checkNumberOfTabs()
      .then(condition => {
          if(condition){
            ...
            setTimeout(function(){this.identifyTabPopup()}, 1000)
          }
      })
  }
  ...
  browser.findByXPATH = function(textClassnameValueOrHref){
    const paths = [
      `self::a[@href='${textClassnameValueOrHref}']`,
    ...
    return this.wait(
      until.elementLocated(
        By.xpath(`//*[${paths[0]} or ${paths[1]} or ${paths[2]} or ${paths[3]} or ${paths[4]} or ${paths[5]} or ${paths[6]}]`)
      ), 2000
    )
  }
